...
export const ProductListing = () => {
  ...
  const category = location.pathname.includes('/men') ? 'men' : 'women';
  ...
  // URL search query (if navigated from search bar)
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setIsMobile(mobile);
      if (mobile) setIsFilterOpen(false); // Close by default on mobile
    };
    handleResize();
  ...
  const filteredProducts = useMemo(() => {
    ...
    return list;
  }, [baseProducts, selectedSubcategories, selectedColors, selectedSizes, priceRange, sortBy, searchQuery]);
  ...
  // Category chips filtering active state logic (Subcategory based tags)
  const isChipActive = (type: 'all' | 'shirts' | 'tshirts' | 'outerwear' | 'footwear' | 'dresses' | 'tops') => {
    if (type === 'all') {
      return selectedSubcategories.length === 0;
    }
    if (type === 'shirts') {
      return selectedSubcategories.length === 1 && selectedSubcategories[0] === 'Shirts';
    }
    if (type === 'tshirts') {
      return selectedSubcategories.length > 0 && selectedSubcategories.every(s => ['T-Shirts', 'Polos'].includes(s));
    }
    if (type === 'outerwear') {
      return selectedSubcategories.length > 0 && selectedSubcategories.every(s => ['Jackets', 'Outerwear', 'Suits'].includes(s));
    }
    if (type === 'footwear') {
      return selectedSubcategories.length === 1 && selectedSubcategories[0] === 'Footwear';
    }
    if (type === 'dresses') {
      return selectedSubcategories.length === 1 && selectedSubcategories[0] === 'Dresses';
    }
    if (type === 'tops') {
      return selectedSubcategories.length > 0 && selectedSubcategories.every(s => ['Tops', 'Blouses'].includes(s));
    }
    return false;
  };

  const handleChipClick = (type: 'all' | 'shirts' | 'tshirts' | 'outerwear' | 'footwear' | 'dresses' | 'tops') => {
    setIsLoading(true);
    setTimeout(() => setIsLoading(false), 300);
    ...
    if (type === 'all') {
      ...
      setSelectedSubcategories(['T-Shirts', 'Polos']);
    } else if (type === 'outerwear') {
      setSelectedSubcategories(['Jackets', 'Outerwear', 'Suits']);
    } else if (type === 'footwear') {
      setSelectedSubcategories(['Footwear']);
    } else if (type === 'dresses') {
      setSelectedSubcategories(['Dresses']);
    } else if (type === 'tops') {
      setSelectedSubcategories(['Tops', 'Blouses']);
    }
  };
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    return count;
  }, [selectedSubcategories, selectedColors, selectedSizes, priceRange]);
  ...
  return (
    <div className="max-w-[1800px] mx-auto px-5 md:px-10 pt-28 pb-24 page-transition-container relative">
      ...
      <div className="flex items-center gap-1.5 text-[9px] text-primary-muted font-sans uppercase tracking-widest mb-4">
        ...
        <span>/</span>
        <span className="text-[#111111] font-bold capitalize">{category}'s Collection</span>
      </div>
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6 border-b border-luxury-border pb-8">
        <div className="max-w-2xl">
          ...
          <h1 className="text-3xl md:text-4xl uppercase font-serif tracking-widest mt-1 text-[#111111] capitalize">
            {category}'s Collection
          </h1>
          <p className="text-xs text-primary-muted font-sans mt-2.5 leading-relaxed">
            Explore our curated range of premium {category}'s apparel and accessories, made with premium fabrics and impeccable cuts.
          </p>
          ...
          <div className="flex flex-wrap gap-2 mt-6">
            {category === 'men' ? (
              // Men's chips
              (['all', 'shirts', 'tshirts', 'outerwear', 'footwear'] as const).map((chip) => {
                ...
                return (
                  ...
                  >
                    {chip === 'tshirts' ? 'T-Shirts & Polos' : chip}
                  </button>
                );
              })
            ) : (
              // Women's chips
              (['all', 'dresses', 'tops', 'outerwear', 'footwear'] as const).map((chip) => {
                const active = isChipActive(chip);
                return (
                  <button
                    key={chip}
      ...
      <div className="border-b border-luxury-border pb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <div ref={searchDropdownRef} className="relative w-full md:w-80 z-30">
          <div className="flex items-center border border-luxury-border px-3.5 py-1.5 bg-luxury-cream/30 hover:border-primary/50 focus-within:border-[#111111] focus-within:bg-white transition-all">
            ...
            <input
              type="text"
              placeholder={`Search ${category}'s collection...`}
              value={searchQuery}
